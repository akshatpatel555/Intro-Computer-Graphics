...
Display()
{
	...
	// Draw and rotate the rear blade
	glPushMatrix();
...
{
	glPushMatrix();
	glTranslatef((GLfloat)0.5, (GLfloat)2.5, (GLfloat)9.0);
...
InitLists()
{
	...
	// Generate the display list for the helicopter
	HeliList = glGenLists(1);
	glNewList(HeliList, GL_COMPILE);

	glPushMatrix();

	// Apply translation and rotations for the helicopter model
	glTranslatef(0.0f, -1.0f, 0.0f);
	glRotatef(97.0f, 0.0f, 1.0f, 0.0f);
	glRotatef(-15.0f, 0.0f, 0.0f, 1.0f);

	// Begin drawing triangles
	glBegin(GL_TRIANGLES);

	// Loop through the helicopter's triangle data
	for (i = 0; i < Helintris; ++i) {
		tp = &Helitris[i];  // Access current triangle
		...
		Unit(n, n);
		n[1] = fabs(n[1]);
		n[1] = (n[1] + 0.25f > 1.0f) ? 1.0f : n[1] + 0.25f;

		float lightFactor = 0.32f * n[1];
	...
	// declatation for heli blades
	BladeList = glGenLists(1);

	glNewList(BladeList, GL_COMPILE);
	glBegin(GL_TRIANGLES);
	glVertex2f(BLADE_RADIUS, BLADE_WIDTH / 2.0);
	glVertex2f(0., 0.);
	glVertex2f(BLADE_RADIUS, -BLADE_WIDTH / 2.0);
	glVertex2f(-BLADE_RADIUS, -BLADE_WIDTH / 2.0);
	glVertex2f(0., 0.);
